...
const [ selectMembers, setSelectMembers] = useState([]);
 const groupName = useInputValidation("")
  ...
  const selectMemberHandler = (id)=>{
    setSelectMembers((prev)=>prev.includes(id) ? prev.filter((currElem)=>currElem !== id): [...prev,id])
  ...
  return (
    <Dialog open>
      ...
      sx={{
        ...
        <Stack justifyContent={'space-evenly'} direction={"row"} gap={"1rem"} marginTop={"1rem"}>
          <Button size='large'  variant='contained' onSubmit={submitHandler} >Create</Button>
